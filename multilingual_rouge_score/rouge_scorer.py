...
from multilingual_rouge_score import scoring
from multilingual_rouge_score import tokenization_wrapper as tokenize
import pyonmttok
import collections
from multilingual_rouge_score.tokenizers import (
	whitespace_tokenize,
	BasicTokenizer,
	LANG2TOKENIZER,
)
from multilingual_rouge_score.stemmers import LANG2STEMMER
import warnings
...
SPECIAL_LANGID_CONVERSION_MAP = {
	l: k for k, v in SPECIAL_LANGID_CONVERSION_MAP.items() for l in v
}
	...
	def __call__(self, text):
		...
		if not tokens and text:
			if self.tokenizer:
				warnings.warn(
					"-"* 5 +
					"No tokens found using inferred tokenizer," +
					" switching to default tokenizer" +
					"-" * 5)
	...
	def __init__(self, lang=None, callable_stemmer=None, min_char_length=3):
		...
		if callable_stemmer:
			...
			if not stemmer_obj:
				self.stemmer = None
				warnings.warn(
					"-" * 5 + f"unknown stemmer language-> {lang}" + "-" * 5)
			else:
